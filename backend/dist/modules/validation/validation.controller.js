...
exports.getValidationErrors = exports.getValidationResults = exports.runValidation = void 0;
 ...
 */
const getValidationResults = async (req, res) => {
    try {
        const fileId = parseInt(req.params.fileId, 10);
        if (!fileId || isNaN(fileId)) {
            return res.status(400).json({ error: 'fileId debe ser un ID numérico válido' });
 ...
 */
const getValidationErrors = async (req, res) => {
    try {
        const fileId = parseInt(req.params.fileId, 10);
        if (!fileId || isNaN(fileId)) {
            return res.status(400).json({ error: 'fileId debe ser un ID numérico válido' });
